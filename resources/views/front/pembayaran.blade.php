        ...
        <div class="container d-md-flex justify-content-between align-content-start">
                ...
                <ul class="cart_main-list">
                    ...
                    @foreach($keranjang as $data)
                        ...
                        <?php
                            ...
                            // $paket_wedding_id_value[] = $data->paket_wedding->id;
                        ?>
                        <input type="hidden" name="paket_wedding_id_all[]" class="paket_wedding_id_all" value="{{ $data->id }}">
                        <input type="hidden" name="jumlah_paket_bayar[]" class="jumlah_paket_bayar" value="{{ $data->jumlah_paket }}">
                            ...
                            <div class="wrapper d-flex flex-wrap flex-xl-nowrap align-items-center justify-content-between">
                                <div class="wrapper_item d-flex align-items-center">
                                    <div class="wrapper_item-media">
                                        <picture>
                                            <source data-srcset="{{ asset('paket_wedding/'. $foto->url) }}" srcset="{{ asset('paket_wedding/'. $foto->url) }}" type="image/webp" style="object-fit: cover !important;" />
                                            <img class="lazy" data-src="{{ asset('paket_wedding/'. $foto->url) }}" src="{{ asset('paket_wedding/'. $foto->url) }}" alt="media" style="height: 140px; object-fit: cover !important;" />
                                        </picture>
                                    </div>
                                ...
                                <div class="price_wrapper price_wrapper--subtotal d-flex flex-column">
                                    ...
                                    <span class="price price--total" id="sub_total{{ $data->id }}">Rp . {{ number_format((float)$jumlah_paket_value, 0, ',', '.') }}</span>
                                </div>
                                <div class="qty d-flex align-items-center justify-content-between">
                                    <span class="d-flex align-items-center" onclick="kurangInCard({{ $data->id }})">
                                        <i class="icon-minus"></i>
                                    </span>
                                    <input class="qty_amount" type="number" readonly value="{{ $data->jumlah_paket }}" min="1" max="99" id="quantityValueCard{{ $data->id }}" />
                                    <span class="d-flex align-items-center" onclick="tambahInCard({{ $data->id }})">
                                        <i class="icon-plus"></i>
                                    </span>
                                </div>
                                <input type="hidden" id="keranjangIdCard{{ $data->id }}" value="{{ $data->id }}">
                                <button type="button" class="close d-flex align-items-center align-items-sm-start" onclick="deleteData(this, {{ $data->id }})">
            ...
            <div class="cart_summary">
                ...
                <form class="cart_summary-form" action="#" method="post" id="bayarPaketForm">
                    ...
                    <input class="cart_summary-form_field field" type="hidden" name="user_id" value="{{ Auth::guard('web')->user()->id }}" />
                    <input class="cart_summary-form_field field" type="text" name="tempat_acara" placeholder="Tempat Acara" />
                    <input class="cart_summary-form_field field" type="date" name="tanggal_acara" placeholder="Tanggal Acara" style="margin-top: 15px;" />
                    ...
                    @foreach($paket_wedding_random as $item)
                            <li class="top_list-item col-lg-6" data-order="1">
                                <div class="top_list-item_wrapper d-flex flex-column flex-sm-row flex-lg-column flex-xxl-row">
                                    <div class="media">
                                        <a href="{{ url('paket-wedding', $item->paket_wedding->slug) }}" rel="noopener norefferer">
                                            <picture>
                                                <source data-srcset="{{ asset('paket_wedding/'. $item->url) }}" srcset="{{ asset('paket_wedding/'. $item->url) }}" type="image/webp" />
                                                <img class="lazy" data-src="{{ asset('paket_wedding/'. $item->url) }}" src="{{ asset('paket_wedding/'. $item->url) }}" alt="media" />
                                            </picture>
                                        </a>
                                    </div>
                                    <div class="main">
                                        <a class="main_title" href="{{ url('paket-wedding', $item->paket_wedding->slug) }}" rel="noopener norefferer"
                                            >{{ $item->paket_wedding->nama_paket }}</a
                                        >
                                        <div class="main_price">
                                            {{-- <span class="price price--old">$45.99</span> --}}
                                            <span class="price price--new">RP. {{ number_format((float)$item->paket_wedding->harga_paket, 0, ',', '.') }} </span>
                                        </div>
                                        {{-- <div class="d-flex"> --}}
  ...
  $("#bayarPaketForm").validate({
        ...
        //     form.submit();
            ...
            $("#bayarPaket").click(function() {
                ...
                if($('#bayarPaketForm').valid()) {
                    ...
                    // var bundle_paket_pemesanan = $(".paket_wedding_all").val();
                    var user_id = $("input[name=user_id]").val();
                    // var jumlah_paket_bayar = $("input[name=jumlah_paket_bayar]").val();
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                           if(data[0] == "berhasil") {
                                window.location.href = '{{ route('pemesanan-wa', Auth::guard('web')->user()->id) }}';
    ...
    $("#keranjangButton").click(function() {
                    ...
                    $.ajax({
                        ...
                        method: "POST",
                        data: {"_token": "{{ csrf_token() }}", paket_wedding_id:paket_wedding_id, user_id:user_id, jumlah_paket:jumlah_paket},
                        success: function(data) {
                            console.log(data);
                           if(data[0] == "berhasil") {
